...
export class AddInstituteComponent implements OnInit {
 ...
 thirdForm: FormGroup;
 forthForm: FormGroup;
 institute = {name:'', logo: null, instituteContact: '', 
  ...
  category=[
    ...
    constructor(private fb:FormBuilder,private api:ApiService,private country:CountryService, 
      private active:ActivatedRoute, private router: Router, private toastrService: NbToastrService) { }
    ...
    ngOnInit() {
      this.active.queryParams.subscribe(data => 
        ...
        this.secondForm = this.fb.group({
          instituteContact: ['',Validators.compose([Validators.required,Validators.pattern(/^([+]?\d{1,2}[.-\s]?)?(\d{3}[.-]?){2}\d{4}/)])],
    ...
    thirdFormSubmit(){
      ...
      if(!this.edit)
      {
        ...
        setTimeout(() => {this.router.navigate(['/pages/home']);
      }, 1000)
